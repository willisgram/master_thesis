...
folder <- "input/"
CleanSheet_round <- data.frame(index = 1:625)
...
for( i in 5:37){
  ...
  CleanSheet_round <- cbind(CleanSheet_round,data_temp$CleanSheets)
  colnames(CleanSheet_round)[i-3] <- paste0("round_",i)
  
}

library(xlsx)
write.xlsx(CleanSheet_round, "output/points_16.xlsx")

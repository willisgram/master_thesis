...
folder <- "input/"
assist_round <- data.frame(index = 1:625)
...
for( i in 5:37){
  ...
  assist_round <- cbind(assist_round,data_temp$Assists)
  colnames(assist_round)[i-3] <- paste0("round_",i)
  
}

library(xlsx)
write.xlsx(assist_round, "output/points_16.xlsx")
